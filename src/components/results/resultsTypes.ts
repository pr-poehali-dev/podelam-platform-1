...
export const MAIN_BLOCK: Record<string, string> = {
  creative: "Перфекционизм и страх оценки — ты не запускаешь, потому что «ещё не готово»",
  help_people: "Неумение ценить себя — ты отдаёшь больше, чем получаешь, и боишься поднять цену",
  analytics: "Паралич анализа — ты ждёшь идеального момента, который никогда не наступит",
  business: "Рассеивание фокуса — слишком много идей одновременно, ни одна не запущена до конца",
  education: "Синдром самозванца — кажется, что ты ещё не готов передавать знания другим",
  communication: "Энергетический слив — ты вкладываешь много в общение, но не конвертируешь это в доход",
  management: "Гиперответственность — делаешь за других, не делегируешь, выгораешь в одиночку",
  practical: "Недооценка своего труда — ты умеешь делать, но не умеешь продавать и называть цену",
  research: "Разрыв между знанием и доходом — ты глубоко разбираешься, но не монетизируешь экспертизу",
  freedom: "Нестабильность модели — ты уходишь от контроля, но не выстроил системный доход взамен",
};

export const MONEY_LOSS: Record<string, string> = {
  creative: "Ты теряешь деньги на неоконченных проектах и низкой цене за свою работу — не доводишь до продажи",
  help_people: "Ты теряешь деньги, беря меньше, чем стоишь, и работая с теми, кто не ценит твою помощь",
  analytics: "Ты теряешь деньги на промедлении — пока анализируешь, возможности уходят к другим",
  business: "Ты теряешь деньги, распыляясь на несколько направлений сразу и не доводя ни одно до прибыли",
  education: "Ты теряешь деньги, откладывая запуск своих продуктов и ставя цену ниже рыночной",
  communication: "Ты теряешь деньги, много работая на видимость, но без чёткой монетизации своей аудитории",
  management: "Ты теряешь деньги, делая за других то, за что должны платить, и не берясь за стратегические задачи",
  practical: "Ты теряешь деньги на заниженном прайсе и отсутствии системы привлечения клиентов",
  research: "Ты теряешь деньги, отдавая знания бесплатно и не упаковывая экспертизу в платный продукт",
  freedom: "Ты теряешь деньги на нестабильности — доход скачет, нет повторных продаж и долгосрочных клиентов",
};

export const INCOME_MODEL: Record<string, string> = {
  creative: "Твоя модель — проектный доход: ведёшь 3–5 клиентов параллельно или продаёшь готовые продукты (шаблоны, гайды, курсы)",
  help_people: "Твоя модель — личная практика: консультации, сопровождение, групповая работа с чётким прайсом и границами",
  analytics: "Твоя модель — экспертный сервис: аудиты, аналитика под заказ, долгосрочные контракты с системными клиентами",
  business: "Твоя модель — масштабируемый бизнес: один продукт, доведённый до автоматических продаж, затем следующий",
  education: "Твоя модель — образовательный доход: курс или программа, которые продаются регулярно, без постоянного личного участия",
  communication: "Твоя модель — медийный доход: аудитория + монетизация через рекламу, партнёрства или продукты своей экспертизы",
  management: "Твоя модель — операционное партнёрство: долгосрочные контракты на управление процессами бизнеса за % или ставку",
  practical: "Твоя модель — мастерская: поток клиентов через репутацию + повышение среднего чека за счёт специализации",
  research: "Твоя модель — платная экспертиза: исследования, консалтинг, публикации, гранты — монетизация глубины знаний",
  freedom: "Твоя модель — портфельный доход: несколько независимых источников, которые работают без твоего постоянного присутствия",
};
