...
export default function TrainerCatalog({ onSelectTrainer }: Props) {
  ...
  return (
    <div className="flex flex-col gap-8 pb-12">
      ...
      {!search.trim() && (
        <div
          onClick={() => navigate("/pro-trainers")}
          className="flex items-center justify-between gap-4 rounded-2xl border border-amber-200/60 bg-gradient-to-r from-slate-900 to-slate-800 px-5 py-4 cursor-pointer hover:shadow-lg hover:shadow-amber-500/10 transition-all duration-300"
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold text-white">PRO-тренажёры</p>
              <p className="text-xs text-slate-400">Стратегия, финансы, логика, симулятор сценариев</p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-xs font-semibold text-amber-400 hidden sm:inline">Смотреть все</span>
            <Icon name="ArrowRight" size={16} className="text-amber-400" />
          </div>
        </div>
      )}
