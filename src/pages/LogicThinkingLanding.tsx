...
import LogicLandingHero from "@/components/logic/LogicLandingHero";
import LogicLandingContent from "@/components/logic/LogicLandingContent";
import LogicLandingPricing from "@/components/logic/LogicLandingPricing";

const TRAINER_ID = "logic-thinking" as const;

const META = {
  title: "Тренажёр логического мышления — ПоДелам",
  description:
    "Развивайте логическое мышление: структура аргументов, причинно-следственные связи, когнитивные искажения. Формульная модель без ИИ. От 990 ₽/мес.",
};
...
export default function LogicThinkingLanding() {
  ...
  useEffect(() => {
    ...
    setMeta("og:url", "https://podelam.su/logic-thinking-info", true);
  ...
  useEffect(() => {
    const refresh = () => {
      ...
      setHasHistory(
        getLogicSessions(TRAINER_ID).some((s) => s.completedAt && s.results)
      );
    };
    refresh();
  ...
  const handlePlanClick = (planId: string) => {
    navigate(`/logic-thinking?plan=${planId}`);
  };
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <LogicLandingHero
        ...
        onNavigateTrainer={() => navigate("/logic-thinking")}
      ...
      <LogicLandingPricing
        ...
        onNavigate={() => navigate("/logic-thinking")}
      />
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="text-center text-sm text-slate-500 space-y-0.5">
              <p>&copy; 2025 ПоДелам. Найди своё дело.</p>
