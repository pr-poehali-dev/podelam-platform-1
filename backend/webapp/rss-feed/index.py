...
def to_rfc822(dt_val) -> str:
    """Конвертирует datetime или строку в RFC 822 формат для RSS"""
    if dt_val is None:
...
def build_turbo_content(body: str, cover_url: str | None, summary: str) -> str:
    """Строит турбо-контент для Яндекс.Дзен"""
    parts = ['<![CDATA[']
    parts.append('<html>')
    parts.append('<head><meta charset="utf-8"/></head>')
    parts.append('<body>')
    ...
    if summary:
        ...
        if clean_summary:
            parts.append(f'<p><b>{html.escape(clean_summary)}</b></p>')

    if body:
        clean_body = body.strip()
        if not clean_body.startswith('<'):
            clean_body = '<p>' + clean_body.replace('\n\n', '</p><p>').replace('\n', '<br/>') + '</p>'
...
def handler(event: dict, context) -> dict:
    ...
    items_xml = []
    for row in rows:
        slug, title, summary, body, cover_url, reading_time, pub_date, meta_desc, category_name = row
        ...
        if cover_url:
            item_parts.append(
                f'      <enclosure url="{html.escape(cover_url)}" type="image/jpeg" length="0"/>'
            )
